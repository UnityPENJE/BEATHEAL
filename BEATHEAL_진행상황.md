...
## ✅ 최근 추가 (상호작용 대체 / 피드백 강화 / 클리어 목표)

### 1. 상호작용 2 대체 — 날아오는 공 쳐내기 (`BonusBall` / `BonusBallSpawner`)
> 기존 "관객 반응"은 시스템 연출이라 평가용 상호작용으로 약해서, **손으로 직접 쳐내는** 상호작용으로 대체.
- **'초 하드' 모드 전용** — 메인은 노트 치기, 공은 초 하드에서만 등장하는 추가 도전(+α)
- 진행(Playing) 중 일정 간격으로 공이 좌우로 흩뿌려져 날아옴 → 팔 뻗기(상지 재활) 유도
- 드럼스틱(`XRController`)·맨손(`Hand`) 둘 다로 쳐냄 → `GameManager.OnBonusBallHit` → 추가 점수(+200)
- 놓쳐도 페널티 없음 (순수 +α 요소). 활성 조건은 `GameManager.BonusBallsEnabled`
- 무대 조명 연출(`OnAudienceReact`)은 보조 효과로 유지

### 2. 직관적 피드백 강화 (`FeedbackFX` + `SfxPlayer`)
- 시각: **파티클 폭발 + "+점수" 팝업 텍스트** (`FeedbackFX`, 프리팹 불필요)
- 청각: **`SfxPlayer`** — 노트 성공 / 노트 실패 / 보너스 공 3종 효과음
  - **2D 재생**이라 거리·위치와 무관하게 항상 들림, AudioListener 없으면 자동 추가
  - 클립을 비워두면 **자동 생성 톤**(에셋 불필요), 인스펙터에 `hitSuccess/hitFail/ballHit` 클립을 넣으면 그 소리로 교체
  - 세팅 시 Casual Game Sounds 팩에서 자동 연결: 성공=DM-CGS-30(상승 차임) / 실패=DM-CGS-02(하강음) / 공=DM-CGS-44(팝) — 파형 분석으로 매칭, 인스펙터에서 교체 가능
- 정답 타격 = 초록 버스트 + 성공음 / 오답 = 실패음 / 보너스 공 = 금색 버스트 + 공 타격음

### 3. 스테이지 클리어 목표 — 목표 라운드 채우기
> (기존 누적 미션 체크리스트는 폐기) 난이도별 **목표 라운드를 모두 완주하면 STAGE CLEAR**.
- 난이도별 목표 라운드: `GameManager.roundGoals = { 쉬움5, 보통8, 어려움10, 초하드12 }` (인스펙터 조정 가능)
- HUD 좌측에 진행도 표시(`UIManager.goalText`): "목표: 라운드 N 클리어 / 진행: x / N"
- 목표 도달 시 배너("🏆 STAGE CLEAR!") + 결과 화면에 `STAGE CLEAR` 표기 (`GameManager.StageCleared`)
- 난이도 `UltraHard` 추가 (타이틀에 "초 하드 (공 날아옴!)" 버튼, 인덱스 3)

> ⚠ 모두 `게임 시스템 + UI 세팅` 버튼 1회 실행으로 자동 연결됨 (기존 씬은 버튼 재실행 필요).
...
## UI — HP 표시 (전용 패널 + 모드별 자동 전환)

- **HP 전용 패널(`HpPanel`)로 분리** — HUD와 별개 오브젝트, `UIManager.hpPanel`로 HUD와 함께 표시/숨김
- **사이먼(소량 HP) = 하트 아이콘 방식**: `maxHP` 개수만큼 HP 구슬 이미지(`hpHearts`)를 만들어 HP가 줄면 **하나씩 사라짐** (fill 아님)
- **리듬(대량 HP=100) = fill 게이지 바**: 아이콘이 불가능하므로 기존 Fill 바(`hpBarRoot`/`hpFill`)로 자동 전환 (빨강→노랑→초록 + 숫자 라벨)
- `UIManager.RefreshHUD`가 `CurrentMaxHP ≤ 아이콘 수`면 아이콘, 아니면 fill 바를 선택해 표시
- 생성: `BeatHealSetup.MakeHearts()`(하트 아이콘) + `MakeHPBar()`(fill 바). 둘 다 HpPanel에 만들어두고 모드별로 켜고 끔
...
2. **`① 게임 시스템(매니저) 세팅`** → **`② UI 세팅`** (순서대로) — 매니저 생성 후 Canvas/버튼/HP/목표 UI 연결
   - 한 번에 하고 싶으면 **`①+② 한 번에 (매니저 + UI)`** 버튼 사용
...
| 6 | 상호작용 2 — 날아오는 공 쳐내기 | ✅ BonusBall/BonusBallSpawner (조명은 보조) |
| 7 | 상호작용 3 — 악기 발광 시퀀스 | ✅ PlaySequence |
| 8 | 피드백 1 — 햅틱 + 색상 + 파티클/팝업 | ✅ TriggerHaptic + ShowCorrect/Wrong + FeedbackFX |
| 9 | 피드백 2 — 효과음(성공/실패/공) | ✅ SfxPlayer (2D, 클립 교체 가능) |
